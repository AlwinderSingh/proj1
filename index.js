...
db.query("Create table if not exists members(Name text , timer integer, age integer, email text primary key, phone integer , timeslot text, password text, newtimeslot text, paymentstatus text)",(err,res)=>{
...
function update(req,res,next){
    if(req.user.timer!=moment().month()+1&&req.user.timeslot!=req.user.newtimeslot){
        db.query(`Update members set timeslot="${req.user.newtimeslot}" where email="${req.user.email}"`,(err)=>{
            console.log(err)
    ...
    if(req.user.timer!=moment().month()+1){
        db.query(`Update members set paymentstatus='false' where email="${req.user.email}"`,(err)=>{
            console.log(err)
...
function CompletePayment(req,res,next){
    db.query(`Update members set paymentstatus="true" where email="${req.user.email}"`,(err)=>{
        res.send("Success")
...
function chpwd(req,res,next){
    ...
    bcrypt.hash(password,17,(err,password2)=>{
        db.query(`Update members set password="${password2}" where email="${req.user.email}"`,(err)=>{
...
function chslot(req,resp,next){
    ...
    bcrypt.compare(password,req.user.password,(err,res)=>{
        if(res){
        db.query(`Update members set newtimeslot="${slot}",timer=${timer} where email="${req.user.email}"`,(err)=>{
...
app.post("/pay",Check,CompletePayment)
app.get("/getpay",Check,getpayfee)
